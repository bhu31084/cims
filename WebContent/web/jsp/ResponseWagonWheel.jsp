...
<%
	...
	fileStr = "/" + fileStr.substring(1, length)
			+ "/images/wagon_wheel.jpg";
	...
	vSpParam.add(type);
	crsDrivenPoints = generateStProc.GenerateStoreProcedure(
			"esp_dsp_WagonWheel", vSpParam, "ScoreDB");
	...
	vSpParam = new Vector();
	vSpParam.add(player1);
	profileDetail = generateStProc.GenerateStoreProcedure(
			"data.esp_dsp_playerProfDetails", vSpParam, "ScoreDB");

	if (profileDetail.next()) {
		if (profileDetail.getObject("bat") != null) {
			rightHanded = profileDetail.getObject("bat").toString()
					.charAt(0);
		}
	}
