...
<%
    ...
    if(session.getAttribute("season") == null){
    	...
    	while(crsObjSeason.next()){
    		if(!crsObjSeason.getString("cur_season_name").equals("")) 
    			season = crsObjSeason.getString("id");    		
    	}
    	session.setAttribute("season",season);
    	vparam.removeAllElements();	
    }else{
    	season = session.getAttribute("season").toString();
    }	
	...
	crsObjGetAssociations = lobjGenerateProc.GenerateStoreProcedure("esp_dsp_associationwise_points",vparam,"ScoreDB"); //need
	vparam.removeAllElements();
	...
	if(crsObjGetFirstSeriesdata != null){
		if(crsObjGetFirstSeriesdata.next()){  /*need to optimise*/
...
<script language="javascript" type="text/javascript"
	src="../js/AssociationAjax.js"></script>
<script>
...
'</div>';
	...
	function start(){
		...
		function getTeamLineUp(matchId){			
			document.frmScorer.action = "/cims/web/jsp/TeamLineUp.jsp?matchid="+matchId;			
		...
		function ShowliveScoreCard(matchid){	
			window.open("/cims/web/jsp/liveScoreCard.jsp?matchid="+matchid,"CIMSweb","location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,top=290,left=690,width=180,height=252");
		}
...
$("#associationtable tr:even").css("background-color", "#f0f7fd");
$("#associationtable tr:odd").css("background-color", "#e6f1fc");
...
<body bottommargin="0" leftmargin="0" topmargin="0"
	onload="start();displaylivescorecard()">
<%}%>
<div id="fb-root"></div>
...
<form method="post" id="frmScorer" name="frmScorer"
...
<table>
	<!-- <br>
			...
			<br>
			<tr>-->
	<td align="center" valign="middle"><b><img
		src="../Image/wait24trans.gif" />&nbsp;<font color="red" size="3">
	Loading ......</b></font></td>
	</tr>
</table>
</div>
...
<script>showPopup('BackgroundDiv','pbar')</script> <jsp:include
	page="Header.jsp"></jsp:include>

<div id="outerDiv"
	style="width: 1000px; background-color: #F4FCFF; float: left">
<table>
	<tr>
		<td valign="top"><%@ include file="commiteeinfo.jsp"%>
		</td>
		<td valign="top"><input type="hidden" name="hidseason"
			id="hidseason" value="<%=season%>"> </input>
		<div id="SecondColDiv" style="width: 600px;">
		<table id="" border="1" style="width: 600px;" class="contenttable">
			<tr>
				<td style="height: 252px; width: 600x; padding-left: 1px;"
					valign="top" colspan=2>
				<table style="height: 252px; width: 600px;" border="0"
					class="contenttable" align="c">
					<tr>
						<td background="../Image/top_bluecen.jpg" colspan="2"
							style="font-size: 16px; color: white; font-weight: bolder; font-family: georgia; padding-left: 7px;">Association</td>
					</tr>
					<tr>
						<td>
						<table style="height: 252px; width: 280px;" border="0"
							class="contenttable" id="AssoNameById" align="c">
							<%if(crsObjGetAssociations != null && crsObjGetAssociations.size()>0){
							    	int colorcounter = 1;%>
							...
							<tr bgcolor="#e6f1fc">
								<%}%>
								<td
									style="text-align: right; padding-right: 7px; font-size: 11px; font-family: Arial;"
									id="<%=colorcounter++%>"><a><IMG id="plusImage"
									name="plusImage" alt="" src="../Image/<%=clublogo%>" /></a></td>
								<td
									style="text-align: left; font-size: 11px; font-family: Arial;">
								<a style="text-decoration: none; color: black;"
									href="javascript:getAssociationData('<%=crsObjGetAssociations.getString("club_id")%>','<%=clubname%>','<%=season%>','<%=clublogo%>');"
									id="dpAssociation" name="dpAssociation"><%=clubname%></a></td>
							...
							<tr>
								<td></td>
								<td colspan="1" style="text-align: right;"><a
									href="/cims/web/jsp/AssociationDetails.jsp" class="serieslinks">More >></a></td>
							</tr>
						...
						<table border="0" align="center" cellpadding="0" cellspacing="0"
							border="0" class="contenttable">
							<tr>
								<td align="left">&nbsp;
								<div id="AssoDivById"
									style="display: none; overflow: auto; height: 210px;"
									class="scrollBarsHV"></div>
								<div id="loadPageDiv" style="overflow: auto; height: 210px;"
									class="scrollBarsHV"></div>
								</td>
							</tr>
						</table>
			...
			<tr>
				...
				<table style="width: 280px; height: 460px;" border="0"
					class="contenttable" id="seriestable">
					<tr>
						<td background="../Image/top_bluecen.jpg" colspan="4"
							style="font-size: 16px; color: white; font-weight: bolder; font-family: georgia; padding-left: 7px;">Tournament</td>
					</tr>

					<%if(crsObjGetSeries != null && crsObjGetSeries.size()>0){ %>

					<tr>
						<td>&nbsp;</td>
						<td style="text-align: left; font-size: 13px;"><b>Tournament
						Name </b></td>
					...
					<tr bgcolor="#e6f1fc">
						<%}%>
						<td
							style="text-align: right; padding-right: 7px; font-size: 11px;"
							id="<%=colorcounter++%>"><a><IMG id="plusImage"
							name="plusImage" alt="" src="../Image/Arrow.gif" /></a></td>
						<td style="text-align: left; font-size: 11px;"><a
							style="text-decoration: none; color: black"
							href="javascript:SeriesMatchPointsTally('<%=SeriesType%>','<%=SeriesDescription%>');showTopBowler('<%=SeriesType%>','<%=SeriesDescription%>')"
							id="dpAssociation<%=SeriesType%>"
							name="dpAssociation<%=SeriesType%>" class="serieslinks"><%=SeriesDescription%></a>
						</td>
						<td
							style="text-align: right; padding-right: 17px; font-size: 12px;"><%=crsObjGetSeries.getString("num_matches_max")%>
						</td>
					</tr>
					<%	
							         }%>
					...
					<tr>
						<td></td>
						<td></td>
						<td colspan="2" align="right"><a href="/cims/web/jsp/seriesDetails.jsp"
							class="serieslinks" id="linkmoresetfocus" name="linkmoresetfocus">More
						>></a></td>
					</tr>
					<%    }else{ %>
				...
				<div id="demo" class="yui-navset"
					style="width: 320px; scrollbar-base-color: #e6f1fc; scrollbar-track-color: #e6f1fc; scrollbar-face-color: #e6f1fc; scrollbar-highlight-color: #e6f1fc; scrollbar-3dlight-color: gray; scrollbar-darkshadow-color: #e6f1fc; scrollbar-shadow-color: gray; scrollbar-arrow-color: black;">
				<ul class="yui-nav">
				...
				<div class="yui-content" style="height: 421px; overflow: auto;"
					class="scrollBarsHV">
				...
				<table style="width: 295px;" border="0">
					...
					<tr style="color: red; font-size: 12">
						<b><%=message%></b>
					</tr>
					<%}else{%>
					...
					<%--	<tr>
													<td align="center" colspan="3">
														<div align="center">
														<%if(gsplayerphoto == null || gsbatsmanphoto == ""){
																<img src="../Image/noimage.gif" width="80" height="80" />											
															<%}else{%>
					...
					<tr>
						...
						<table style="width: 295px;" border="0">
							...
							<%-- <tr>
															<td align="center" colspan="3">
																<div align="center">
																<%if(gsbatsmanphoto == null || gsbatsmanphoto == ""){%>
																	<img src="../Image/noimage.gif" width="80" height="80" />											
																<%}else{%>
				...
				<div>
				<table style="width: 295px;" border="1" id="tlb11">
					<tr>
						<td>
						...
						<table border="0" style="width: 282px;" align="center">
							...
							<tr bgcolor="#e6f1fc">
								<%}%>
								<td align="center" id="<%=colorcounter++%>"><IMG
									id="plusImage<%=gsteamId%>" name="plusImage<%=gsteamId%>"
									alt="" src="../Image/Arrow.gif" /></td>
								<td align="center" nowrap="nowrap"><a
									href="javascript:ShowTeamPositionDetailDiv('<%=gsteamId%>','<%=crsObjFirstSerPt.getString("series")%>')"><%=crsObjFirstSerPt.getString("team_name")%></a></td>
							...
							<tr>
								<td colspan="7">
								<div id="ShowMatchPtDetailsDiv<%=gsteamId%>"
							...
							<tr>
								<td colspan="7">
								<div id="tosetfocus"><b>Click On Team name To get The
								Match Details </b></div>
								</td>
							</tr>
						</table>
			...
			<tr>
				...
				<div id="container"
					style="width: 280px; height: 575px; overflow: auto;"
					class="scrollBarsHV"></div>
				</td>
				<td valign="top">
				<table>
					<tr>
						<td style="width: 310px;" valign="top"><table style="width:310px;height: 220px;" border="0" class="contenttable">		
											   <tr >
											 	<td height="10%" background = "../Image/top_bluecen.jpg" colspan="4" style="font-size: 16px;color: white;font-weight: bolder;font-family: georgia;padding-left: 7px;" >Advertisements</td>
											   </tr>
												<tr>
													<td >
														<ilayer width=&{swidth}; height=&{sheight}; name="slider1" bgcolor=&{sbcolor};>
														<layer name="slider2" width=&{swidth}; onMouseover="sspeed=0;" onMouseout="sspeed=restart">
														</layer>
														</ilayer>
														<script language="JavaScript">if (document.getElementById || document.all){document.write('<span style="height:'+sheight+';"><div style="position:relative;overflow:hidden;width:'+swidth+';height:'+sheight+';clip:rect(0 '+swidth+' '+sheight+' 0);background-color:'+sbcolor+';" onMouseover="sspeed=0;" onMouseout="sspeed=restart"><div id="slider" style="position:relative;width:'+swidth+';"></div></div></span>')}</script>
							 	     			      	...
							 	     			      	<tr>
							 	     			      	    <td align="center" style="background-color: #F4FCFF">
														   	   	<table style="width: 300px;" class="contenttable" class="scrollBarsHV">
														   	   	<tr>
														...
														<tr>
														   <td>
																<!-- <div class="fb-comments" data-href="http://www.bccicricket.org/cims" data-num-posts="100" data-width="270"></div>-->
														   </td></tr>
				</table>
				</td>
			</tr>
		...
		<table border="1" class="contenttable"
			style="width: 172px; height: 282px;">
			<tr style="height: 10px;">
				<td colspan="2"
					style="font-size: 11px; color: white; font-weight: bolder; width: 180px;"
					valign="top"><img src="../Image/live-scores.gif" /></td>
			</tr>
			<tr style="height: 10px;" bgcolor="#f0f7fd">
				<td colspan="2" style="font-size: 12px; color: white; width: 180px;"
					valign="top"><img id="matchplusImage" src="../Image/Arrow.gif"
					style="text-decoration: none;" /><a
					href="<%=common.getBcciTvLink() %>" style="text-decoration: none;">
				Show Live Series</a></td>
			</tr>
			<tr>
				<td valign="top" colspan="2">
				<div id="matchesDiv" style="display:block">
...
<%					}else{
						...
						while(crsObjTodayMatches.next()){ 
							...
							getmatchId = crsObjTodayMatches.getString("series");
							if(mcounter % 2 == 0 ){%>
							<tr bgcolor="#f0f7fd">
<%							}else{
%>							<tr bgcolor="#e6f1fc">
...
%>
								<td id="<%=mcounter++%>" nowrap="nowrap" colspan="2">
									 <a href="<%=common.getBcciTvLink() %>" style="text-decoration: none;" id="<%=crsObjTodayMatches.getString("series")%>"><%=crsObjTodayMatches.getString("name")%></a>
									<!-- <a href="/cims/media?login_page_series_id=<%=crsObjTodayMatches.getString("series")%>" style="text-decoration: none;" id="<%=crsObjTodayMatches.getString("series")%>"><%=crsObjTodayMatches.getString("name")%></a>-->
								</td>
							</tr>
<%						}
%>
...
%>			<tr style="color: red; font-size: 12">
				<td colspan="4" valign="top"><b>No Match Scheduled ForToday</b></td>
			</tr>
<%			}
%>
		</table>
		</td>
		<!-- third column/div end -->
	</tr>
</table>
<!-- outer main table end --></div>
	...
	function getlivematchscorecard(matchid){
		...
		if(xmlHttp==null){
			...
			return;
				...
				var url;
		    	url="/cims/web/jsp/getlivematchresponse.jsp?matchid="+matchid;
...
<%--	}--%>
	...
	function CheckUserId(){	
		if(document.getElementById("txtUserName").value != ''){
			...
			window.open('/cims/jsp/ForgotPassword.jsp?txtUserName='+txtUserName,'forgotPass','top= 50,left = 50,location=no,directories=no,status=yes,menubar=no,scrollbars=no,resizable=no,width=500,height=300');			
		}else{
	...
	function ShowDesktopScoreCard(matchId){		
		window.open('/cims/web/jsp/ScorecardSummary.jsp?matchid='+matchId,'inningdetail','top= 50,left = 50,location=no,directories=no,status=yes,menubar=no,scrollbars=no,resizable=no,width=420,height=380');
	}
