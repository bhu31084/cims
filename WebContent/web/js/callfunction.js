	...
	this.setpatenership = function(){
	    patenershipball = $$('hdcurrentpatnershipball');
	}
	this.resetpatenership = function(){
	    patenershipball = 0;
	}
	this.setPatenershipBall = function(ball){
	    patenershipball = parseInt(patenershipball) + parseInt(ball);
	}
	this.getPatenershipBall = function(){
	    return patenershipball;
	}
	this.addPitch = function(id){
	    try{
	        pitchpostion =id;
	        pitchFlag = true;
	    }catch(err){
	...
	this.callFunction = function (type,run ){
	    try{
	        //  this.convertData();
	        ...
	        if(type=="ball"){  //  Function For  Ball + runs This Condition for 1 run to 8 run
	            var r = confirm("You have selected "+ run +" Runs. Do you want to continue?");
	            if (r == true) {
	        ...
	        }else if(type=="sixruns"){ // this logic for if click on link foure
	        var r = confirm("You have selected "+ run +" Runs. Do you want to continue?");
	        if (r == true) {
	            landmarkflag = true; // this flag indicate ball is not increase
	            this.setPatenershipBall(1);
	...
	hideMenu();
	var r = confirm("You have selected Wide Ball With "+ run +" Runs. Do you want to continue?");
	if (r == true) {
	    landmarkflag = false; // this flag indicate ball is not increase
	    for(i=0;i<leng;i++){
	        newData.flag = '6';
	...
	showState('noballbatplussubmenuboudry');
	    ...
	    if($$('selType')=='NoBallRun'){
	        var r = confirm("You have selected "+run+" runs with NoBall.Do you want to continue?");
	    }else{
	    var r = confirm("You have selected "+run+" Extra runs with NoBall.Do you want to continue?");
	}
	if (r == true) {
	    landmarkflag = false; // this flag indicate ball is not increase
	    this.setPatenershipBall(1);
	...
	var r = confirm("You have selected "+run+" runs with No Ball.Do you want to continue?");
	if (r == true) {
	    landmarkflag = false; // this flag indicate ball is not increase
	    this.setPatenershipBall(1);
	    for(i=0;i<leng;i++){
	...
	var r = confirm("You have selected "+run+" byes runs.Do you want to continue?");
	if (r == true) {
	    landmarkflag = true; // this flag indicate ball is not increase
	    this.setPatenershipBall(1);
	    newData.flag = '5'; // this flag is use for insert runid and extra id in ball_runtype_map  table
	...
	showState('legbyessubmenu');
	    ...
	    var r = confirm("You have selected "+run+" leg byes runs.Do you want to continue?");
	    if (r == true) {	
	        landmarkflag = true; // this flag indicate ball is not increase
	        this.setPatenershipBall(1);
	        newData.flag = '5'; // this flag is use for insert runid and extra id in ball_runtype_map  table
	...
	// this.convertData();
	   landmarkflag = true; // this flag indicate ball is not increase
	   patenershipball = 0;
	   document.getElementById('hdmatchtime').value = "0";	
	...
	}else if(type=="Absent Hurt"){
	hideMenu();
	var r = confirm("You have selected Absent Hurt Option .Do you want to continue?");
	if (r == true) {
	...
	this.dismissalType(id,batsmanStriker,ballObj.bowlerSetStriker(),"0"); //
	res = this.callAjex();
	scoreObj.updateWicket(1);
	if($('Wickt').innerHTML!="10"){
	...
	closePopup('BackgroundDiv','PopupDiv');
	    ...
	    this.dismissalType(id,wicket.batNameId,wicket.filder1Id,wicket.filder2Id); //dismissal_filder[0] is id of Filder
	...
	closePopup('BackgroundDiv','PopupDiv');
	    ...
	    if(selbatName=="0"){
	        this.dismissalType(id,bastmenObj.batSetStriker(),"0","0"); //0 for strierk is retire out
	    }else{
	    this.dismissalType(id,bastmenObj.batSetNonStriker(),"0","0"); // 1 for non striker is retire out
		}
	...
	var selbatName =  $$('selbatName');
	if(selbatName=="0"){
	    this.dismissalType(id,bastmenObj.batSetStriker(),"0","0"); //0 for strierk is handle the ball
	}else{
	this.dismissalType(id,bastmenObj.batSetNonStriker(),"0","0"); // 1 for non striker is handle the ball
	}
	...
	closePopup('BackgroundDiv','PopupDiv');
	    ...
	    if(selbatName=="0"){
	        this.dismissalType(id,bastmenObj.batSetStriker(),"0","0"); //0 for strierk is handle the ball and obstructing the field
	    }else{
	    this.dismissalType(id,bastmenObj.batSetNonStriker(),"0","0"); // 1 for non striker is handle the ball and obstructing the field
	}
	bastmenObj.extraUpdate(0,'wide');
	scoreObj.extraUpdate(0);
	ballObj.extraUpdate(0,'wide');
	...
	closePopup('BackgroundDiv','PopupDiv');
	    ...
	    if(selbatName=="0"){
	        this.dismissalType(id,bastmenObj.batSetStriker(),"0","0"); //0 for strierk is no ball +  handle the ball and obstructing the field
	    }else{
	    this.dismissalType(id,bastmenObj.batSetNonStriker(),"0","0"); // 1 for non striker is  no ball handle the ball and obstructing the field
	}
	bastmenObj.extraUpdate(0,'noball');
	scoreObj.extraUpdate(0);
	ballObj.extraUpdate(0,'noball');
	...
	this.showsubmenu = function(MainDivId,SubDivId,event){
	    ...
	    if(screen.width != 1024 || screen.height != 768){	
	        ...
	        document.getElementById(SubDivId).style.left = parseInt(maindiv) + parseInt(130); 
	    } 
	}
	
	this.callAjex = function (){
	    try{
	...
	this.dismissalType = function(dismissal_id,batsman,dismissalBy1,dismissalBy2){
	    try{
	...
	if (r == true) {
	    ...
	    this.chkonlinoffline();
	    if(type!="ajaxinterval"){
	        if (type=="Drinks" || type=="Injury" || type=="Change Ball"){
	            bastmenObj.setmatchtimerflag("start");
	        }else{
	        bastmenObj.setmatchtimerflag("stop");
	    }
	...
	if(type=="ajaxinterval"){ // this logic to update end time
	    ...
	    document.getElementById("main").action = "scorerrefresh.jsp"
	...
	this.checkintervalrefreshstatus = function(){
	...
	this.penalty = function(id,run,name){
	    try{
	        hideMenu();
	        var r = confirm("You have selected "+ name +" Option .Do you want to continue?");
	        if (r == true) {	
	            ...
	            if(response){
	                ...
	                newData.description = $$('txtpenaltyremark');
	                document.getElementById("txtpenaltyremark").value="";
	                if(($$('hdmatchtype')=="oneday" && $$('inningNo')=="2") || ($$('hdmatchtype')=="test" && $$('inningNo')=="4")){
	...
	this.mousemove = function(event){
	    try{
	        if(window.event.keyCode == 32){
	            if(divname!=""){
	                $(divname).style.left = event.x;
	                $(divname).style.top = event.y;
	            }
	        }
	    }catch(err){
	...
	this.caughtwktshow = function(type,id){
	    ...
	    var wicketkeeperarray = (document.getElementById("wktselFldName").value).split("~");
	...
	this.adjusttime = function(id){
	    ...
	    if(enddate == ''){
	        ajexObj.sendupdateIntervalTime(intervalid,startdate,enddate,hdstartdate,"1");
	    }if(isOkPrevDate){
	    if(isOkCurrDate){
	        ajexObj.sendupdateIntervalTime(intervalid,startdate,enddate,hdstartdate,"1");
	    }
	...
	this.edittime = function(id){
	    ...
	    var isOkPrevDate = validatewithpreviousdate(enddate,startdate,flag)
	    if(enddate == ''){
	        ajexObj.sendupdateIntervalTime(intervalid,startdate,enddate,hdstartdate,"2");
	    }if(isOkPrevDate){
	    if(isOkCurrDate){
	        ajexObj.sendupdateIntervalTime(intervalid,startdate,enddate,hdstartdate,"2");
	    }
	...
	this.editwicketBalltime = function(wktid,batsmanid){ 
	    ...
	    var wicketNextBallTime = document.getElementById("txtwicketnextball"+wktid).value;
	    var inningid =  document.getElementById('inningId').value
	    var isOkCurrDate = validatewithcurdate(wicketNextBallTime,wicketBallTime)
	    var flag = 2//for alert message
	    var isOkPrevDate = validatewithpreviousdate(wicketNextBallTime,wicketBallTime,flag)
	    
	    if(wicketBallTime.indexOf("-") == -1  || wicketNextBallTime.indexOf("-") == -1){
	        if(wicketNextBallTime == ''){
	            if(wicketBallTime.indexOf("-") == -1){
	                ajexObj.sendupdateBallTime(batsmanid,inningid,wicketBallTime,wicketNextBallTime);
	...
	this.editInningTime = function(inningid){
	    ...
	    var flag = 1//for alert message
	    var isOkPrevDate = validatewithpreviousdate(inningEndTime,inningStartTime,flag)
	    if(inningStartTime.indexOf("-") == -1  || inningEndTime.indexOf("-") == -1){
	        
	        if(inningEndTime == ''){
	            isOkCurrDate = validatewithcurdate(inningStartTime,inningStartTime)
	            if(isOkCurrDate){
	                if(inningStartTime.indexOf("-") == -1){
	                    ajexObj.sendupdateInningStartTime(inningid,inningStartTime,inningEndTime);
	                    alert("Date and Time saved")
	                }
	            }
	...
	this.setresult = function(){
	    ...
	    window.opener="";
	    window.close();
	    window.open("/cims/jsp/SetSecondInning.jsp?","CIMS2","location=Yes,directories=Yes,status=yes,menubar=Yes,scrollbars=Yes,resizable=yes,top=0,left=0,width="+(window.screen.availWidth-20)+",height="+(window.screen.availHeight-20));	
	    
	}	
	this.goToLogin = function(){
	    try{
	        document.main.action = "/cims/web/jsp/Login.jsp"
	...
	this.newBatsmanSelection = function(selbat){
	    ...
	    playerArr[1];	
	    var r = confirm("Do you want to select (" + playerArr[1] + ")  as new batsman.");
	    if(r==true){
	        insRow('BATT_TABLE');
	    }
	} 
	...
	this.DiplayReportForInningOne = function(pos) {
	    ...
	    var flg = "P";
	    var reportflag = 'Y'
	    if(document.getElementById('str'+pos).value!=null && document.getElementById('str'+pos).value!=""){
	        var iningOneId = document.getElementById('str'+pos).value;
	        window.open("/cims/jsp/previousInningsDetail.jsp?InningIdPre="+iningOneId+"&flg="+flg,"scorecard","location=no,directories=no,status=yes,menubar=no,scrollbars=no,resizable=no,top=20,left=20,width=810,height=630");
	    }
	}
	...
	this.setmatchpoint = function(){
	    window.open("/cims/jsp/MatchPoints.jsp?","CIMS3","location=Yes,directories=No,status=yes,menubar=No,scrollbars=Yes,resizable=Yes,top=90,left=90,width=550,height=400");	
	}
	...
	this.updateOverRuns = function (ballNo,ballid,runs,wideball,noball,legbyes,byes,wicket,overno,date,batsman,bowler,bowlerid){
	    showPopup('BackgroundDiv', 'updateRunsDiv' );
	    ajexObj.sendDataUpdateOverRuns(ballNo,ballid,runs,wideball,noball,legbyes,byes,wicket,overno,date,batsman,bowler,bowlerid);
	    if(wicket == "Y"){
	...
	ajexObj.sendDataUpdateRow(ballid,runType,wideBallType,noBallType,legByesType,byesType,formattedDate,over,bowlerid);
	}
	...
	*/
	...
	this.viewPlayers = function(matchid){
	    window.open("/cims/jsp/ViewTeamPlayersList.jsp?match="+matchid,"playerlist","location=no,directories=no,status=no,menubar=no,scrollbars=Yes,resizable=Yes,top=100,left=100,width=800,height=450");	
	}
	...
	this.updateRowWicket = function(ballid){
	    ...
	    ajexObj.sendDataUpdateWicketDetail(ballid,wicketType,fielder1,fielder2,Striker);
	...
	this.addSecdate = function(date){
	    ...
	    if(datearr.length >1){
	        ...
	        var min = timearr[1];
	        var ss = timearr[2];
	        var caldate  = year + "-" + month + "-" + day + " " + hr + ":" + min + ":" + ss;
	        return caldate;
	    }else{
	    return date;
	}
