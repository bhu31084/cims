	...
	function getLastTenMatches(prePage){
		...
		function searchMatches(pageNo){
			...
			if(document.getElementById("txtMatchID").value != null && document.getElementById("txtMatchID").value != "0" && document.getElementById("txtMatchID").value != ""){
				...
				document.frmScorerMatchSetUp.action = "/cims/jsp/admin/MatchAnalysisSelection.jsp";
			...
			document.getElementById("btnSave"+matchId).disabled = false;						
	       	if(document.getElementById("hddivmatchid").value == 0){
				...
				if (xmlHttp==null){
					...
					var url ;									
					url="/cims/jsp/admin/MatchAnalysisSelectionResponse.jsp?matchId="+matchId;					
					document.getElementById("MatchAnalystDiv"+matchId).style.display='';
					document.getElementById("MatchAnalyst1Div"+matchId).style.display='';
					document.getElementById("AnalystDiv"+matchId).style.display='none';
					document.getElementById("Analyst1Div"+matchId).style.display='none';
		            ...
		            if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete"){
						...
						var mdivAnalyst1 = document.getElementById("MatchAnalyst1Div"+matchId);
						mdivAnalyst.innerHTML = cmbbox[0];
						mdivAnalyst1.innerHTML = cmbbox[1];
					}
				}
	       	}else{
	       		if(SaveMatchData(document.getElementById("hddivmatchid").value)==false){
			        ...
			        if (xmlHttp==null){
						...
						hdscorer = document.getElementById("hdanalyst"+matchId).value;
						hdscorerNm = document.getElementById("hdanalystNm"+matchId).value;
						hdscorer2	= document.getElementById("hdanalyst1"+matchId).value;
						hdscorer2Nm	= document.getElementById("hdanalyst1Nm"+matchId).value;
			           	var url ;
						url="/cims/jsp/admin/MatchAnalysisSelectionResponse.jsp?matchId="+matchId;						
						document.getElementById("MatchAnalystDiv"+matchId).style.display='';
						document.getElementById("MatchAnalyst1Div"+matchId).style.display='';
	...
	/*	function stateChangedUmpResponse(){
		...
		function SaveMatchData(matchId){				
	            ...
	            if (xmlHttp==null){
					...
					var url ;	                
	                url="/cims/jsp/admin/MatchScorerSaveRecordResponse.jsp?matchId="+matchId+"&analyst="+analyst+"&analyst1="+analyst1+"&scorer="+scorer+"&scorer2="+scorer2+"&gsflag=analysis";
	...
	/*	function stateChangedScorerResponse(){
		...
		function PrintMatchOfficialsOfferLetters(matchId){			
			var r = confirm("Do you Want to view AppointMent Letters ?");
				if (r == true){
					window.open("/cims/jsp/admin/PrintAnalysisOfficialLetter.jsp?matchId="+matchId+"&flag=1","CIMS","location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,top=100,left=100,width=880,height=500");
				}
				else{
					document.frmScorerMatchSetUp.action = "/cims/jsp/admin/MatchAnalysisSelection.jsp";
...
<div class="leg">Analysis Selection</div>
<%--    Venue Master--%>
<div >
			 	...
			 	<table align="center" width="100%">
					<tr>
						<ul class="pagination">
							...
							<a id="btnBack" name="btnBack" class="btn btn-small" style="text-decoration:none;" href="javascript:getLastTenMatches('<%=Integer.parseInt(pageNo)%>')"><<</a>
							&nbsp;<a id="btnNext" name="btnNext" class="btn btn-small" style="text-decoration:none;" href="javascript:getFirstTenMatches('<%=Integer.parseInt(pageNo)%>')">>></a>
						</ul>
					<td colspan="10"></td>	
					</tr></table>
			 	<table id="tablematches" width="100%">
					<tr class="contentDark">
						...
						<td class="colheadinguser">Main Analyst </td>
						<td class="colheadinguser">Assistant Analyst</td>	
				...
				%>
					<tr >
						...
						<td align="center"><font size="2">
							<div id="AnalystDiv<%=crsObjMatchData.getString("match_id")%>"><%=crsObjMatchData.getString("analyst")%></div></font>
							<div id="MatchAnalystDiv<%=crsObjMatchData.getString("match_id")%>" style="display: none;"></div>					
							<input type="hidden" id="hdanalystNm<%=crsObjMatchData.getString("match_id")%>" name="hdanalystNm<%=crsObjMatchData.getString("match_id")%>" value="<%=crsObjMatchData.getString("scorer")%>" >
							<input type="hidden" id="hdanalyst<%=crsObjMatchData.getString("match_id")%>" name="hdanalyst<%=crsObjMatchData.getString("match_id")%>" value="<%=crsObjMatchData.getString("scorerid")%>" >
						</td>
						<td align="center"><font size="2">
							<div id="Analyst1Div<%=crsObjMatchData.getString("match_id")%>"><%=crsObjMatchData.getString("analyst1")%></div></font>
							<div id="MatchAnalyst1Div<%=crsObjMatchData.getString("match_id")%>" style="display: none;"></div>					
							<input type="hidden" id="hdanalyst1Nm<%=crsObjMatchData.getString("match_id")%>" name="hdanalys1tNm<%=crsObjMatchData.getString("match_id")%>" value="<%=crsObjMatchData.getString("scorer")%>" >
							<input type="hidden" id="hdanalyst1<%=crsObjMatchData.getString("match_id")%>" name="hdanalyst1<%=crsObjMatchData.getString("match_id")%>" value="<%=crsObjMatchData.getString("scorerid")%>" >
						</td>
						<td width="3">
							<input class="btn btn-warning" type="button"  id="btnSave<%=crsObjMatchData.getString("match_id")%>" align="center" value="Save" disabled="disabled" onclick="javascript:SaveMatchData('<%=crsObjMatchData.getString("match_id")%>')">
						</td>
						<td align="right" width="3">
							<input  class="btn btn-warning" type="button"  id="btnPrintOfficial<%=crsObjMatchData.getString("match_id")%>" align="right" value="Get App. letter" title="Get Appointment Letter" onclick="javascript:PrintMatchOfficialsOfferLetters('<%=crsObjMatchData.getString("match_id")%>')">
						</td><%														
						}							
					}//end of while
				}//end of outer if
