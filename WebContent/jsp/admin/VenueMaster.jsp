...
<%
	   ...
	   //Variables for venue master
		...
		GenerateStoreProcedure lobjGenerateProc = new GenerateStoreProcedure();
          ...
          try {
			if (request.getParameter("hiddenval") != null && request.getParameter("hiddenval").equals("submit")) {
				if(request.getParameter("hditemlist6").equals("")){
					serverMessage = "Association selection is mandatory.";
				}else{                
		...
		/***************************************************************************************/
				if(serverMessage.equals("")) {
					...
					crsObjResultvenue = lobjGenerateProc.GenerateStoreProcedure("esp_amd_venuessmst", vparam, "ScoreDB");
					vparam.removeAllElements();
				}
			}
		
			vparam.add("1");
			crsObjResultState = lobjGenerateProc.GenerateStoreProcedure("esp_dsp_statesval", vparam, "ScoreDB");
			vparam.removeAllElements();
			
			vparam.add("0");
			crsObjVenuesearch = lobjGenerateProc.GenerateStoreProcedure("esp_dsp_venuesearch", vparam, "ScoreDB");
			vparam.removeAllElements();
...
<script type="text/javascript">
	...
	function callNextPage(id){
		...
		document.regionMaster.action = "/cims/jsp/admin/VenueMaster.jsp";
	 ...
	 function textValidatevenue(){
		...
		if(document.getElementById('selClub').value==""){
			mess=mess+  " Select Association \n";
			flag= false;
		}
		...
		return flag;
		 ...
		 function cancellation(){
			...
			document.getElementById('selClub').value = "";		 
    ...
    function SelObj(formname,selname,textname,str) {
      ...
      function bldUpdate4(thisEvent) {
    		...
    		if(thisEvent.keyCode  == 40){
    			selected("itemlist4");
    	    }   
    	}
  	      
        function bldUpdate5(event) {
        var str = document.forms[this.formname][this.textname].value.replace('^\\s*','');
        // If there is an empty String, don't search (hide list)
        if(str == '') {
          this.bldInitial();
     ...
     function update6(event) {
       ...
       function showList4(event) {
	        ...
	        if(document.getElementById("itemlist4").value=="0" ||document.getElementById("itemlist4").value=="" ||event.keyCode == 0){
	        	document.getElementById("lister4").style.display="block";
	        }
     	...
     	function showList5(event) {
        	...
        	if(document.getElementById("itemlist5").value=="0" ||document.getElementById("itemlist5").value==""||event.keyCode == 0){
        	 	document.getElementById("lister5").style.display="block";
        	}
         	document.regionMaster.hditemlist5.value=document.regionMaster.itemlist5.value;
      	}
      ...
      function showList6(event) {
	      ...
	      if(document.getElementById("itemlist6").value=="0" ||document.getElementById("itemlist6").value==""||event.keyCode == 0 ){
	         document.getElementById("lister6").style.display="block";
	      }
      }
	...
	function hideList6() {
        ...
        if(valName!=""){
	        var url = "/cims/jsp/admin/VenueMasterfromcombo.jsp?VenueId="+valName+"&response=search";
			xmlHttp.onreadystatechange = displayData;
			xmlHttp.open("get",url,false);
			xmlHttp.send(null);	
	    }    
...
function GetXmlHttpObject(){
    ...
    function showrecord()
      {         
       ...
       if(valName!="0"){
        var url = "/cims/jsp/admin/VenueMasterfromcombo.jsp?pageid="+valName+"&response=page"+"&status="+status;
		xmlHttp.onreadystatechange = displayData;
		xmlHttp.open("get",url,false);
		xmlHttp.send(null);
...
<div class="leg">Venue Master</div>
<%--    Venue Master--%>
...
<%--	search Venue	----%>
           <tr align="left" class="contentDark">
				...
				<input class="textBoxAdminSearch" type="text" id="entry6"	name="entry6" size="30" onKeyUp="javascript:obj7.bldUpdate6(event);">
				 <input  class="btn btn-small"	id="show2" type="button" value="V" onClick="changeList6(event);">
 ...
 <%----search  Venue end--%>
			...
			<tr align="left" class="contentLight">
				<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Address:</td>
...
<%--	state	venue	----%>
			<tr align="left" class="contentLight">
				...
				<td>
				<input class="textBoxAdmin" type="text" id="entry4"	name="entry4" size="30" onKeyUp="javascript:obj5.bldUpdate4(event);" onfocus = "this.style.background = '#FFFFCC'"  onblur = "changeColour(this)"> 
				 <input  class="btn btn-small"	id="show2" type="button" value="V" onClick="changeList4(event);">
				 	<a  href="/cims/jsp/admin/StateMaster.jsp" class="btn btn-warning" style="text-decoration:none;">Add State</a>				 
				<DIV align="left" style="width:250px">
...
<%--	state end	venue	----%>
				...
				<td>
				<input class="textBoxAdmin" type="text"	id="entry5" name="entry5" size="30" onKeyUp="javascript:obj6.bldUpdate5(event);" onfocus = "this.style.background = '#FFFFCC'"  onblur = "changeColour(this)">
				 <input  class="btn btn-small"	id="show2" type="button" value="V" onClick="changeList5(event);">
				<a  href="/cims/jsp/admin/LocationMaster.jsp" class="btn btn-warning" style="text-decoration:none;">Add City and Map</a>		
				<DIV align="left" style="width:250px">
