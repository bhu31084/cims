	    ...
	    function displayMatchId(){
	    	if(document.getElementById("hdSubmit").value != ""){
	    		...
	    		if(ans == "success update"){
	    			...
	    			if (r == true)
            		{            
                		document.frmMatchSetUp.action = "/cims/jsp/admin/AdminMatchSetUp.jsp";
			        }else{
                		document.frmMatchSetUp.action = "/cims/jsp/Logout.jsp";						
                    }
                document.frmMatchSetUp.submit();
	    		}else{	    
		    		if(document.getElementById("hdmflag").value=="3"){
			    		alert(ans)
		    		}else{			
		    			var r = confirm("Your Match Id "+ans+" Has Been Generated.Do you Want to Add Another Match?");
	            		if (r == true)
	            		{
	                		document.frmMatchSetUp.action = "/cims/jsp/admin/MatchSetUpMaster.jsp";
		                }else
	    	            {
	                		document.frmMatchSetUp.action = "/cims/jsp/Logout.jsp";
			        	}
			...
			if(document.getElementById("chkprevious").checked == true){
				...
				if(document.getElementById('dpvenue').value == "" ){
					...
					if(flag){
						...
						document.frmMatchSetUp.action ="/cims/jsp/admin/MatchSetUpMaster.jsp?seasonId="+seasonId+"&Scorer1="+ScorerId1+"&Scorer2="+ScorerId2+"&tournamentid="+TournamentId+"&venueId="+venueId+"&team1Id="+team1Id+"&team2Id="+team2Id+"&MtchCategoryId="+MtchCategoryId+"&MtchTypeId="+MtchTypeId;
						document.frmMatchSetUp.submit();
					}
				}
			}else{
...
<%}%>
	...
	<div class="leg">Match Detail Form</div>
<%--    Venue Master--%>
...
%>							    </select>
						...
						<tr width="100%" align="right" class="contentDark">
							<td colspan="4" height="24">       			
				       			<input class="btn btn-warning" type="button"  id="btnsubmit" name="btnsubmit" value=" Submit" onclick="callNextPage()">
				       			<%if(editMatchId != null){%>
				       			<%}else{%>
				      			...
				      			<input  type="hidden" id="hdmflag" name="hdmflag" value="<%=mflag%>">
