...
<%
	...
	GenerateStoreProcedure  lobjGenerateProc = new GenerateStoreProcedure();
		...
		try{
			...
			vParam.add("");
			vParam.add(flag);
			crsObjMatchPts = lobjGenerateProc.GenerateStoreProcedure("esp_dsp_scorermatchdtls",vParam,"ScoreDB");
...
<%
		...
		try{
			...
			vParam.add(dflag);
			crsObjscorermatchdtls = lobjGenerateProc.GenerateStoreProcedure("esp_dsp_scorermatchdtls",vParam,"ScoreDB");
			vParam.removeAllElements();
		}catch (Exception e) {
			System.out.println("Exception"+e);
	 		e.printStackTrace();
	...
	<table width="880" border="1" align="center" cellpadding="2" cellspacing="1" class="table">
   		<tr class="contentLight" >   			
			...
			int dcounter = 1;
				...
				getmatchId = crsObjscorermatchdtls.getString("match_id");
				if(dcounter % 2 != 0){%>
		<tr class="contentDark">
<%				}else{
%>		<tr class="contentLight">
<%				}%>
			...
			<td id="<%=dcounter++%>" width="5%" align="center">Not Assigned</td>
<%			}else{
%>
			<td width="5%" align="center"><%=crsObjscorermatchdtls.getString("Scorer_2")%></td>
			<%}%>
			<td width="5%" align="left"><%=crsObjscorermatchdtls.getString("series_season_name")%></td>
			<td width="5%" align="left"><a href="javascript:ShowFullScoreCard('<%=getmatchId%>')" ><%=crsObjscorermatchdtls.getString("matchbtwn")%></a></td>
