...
<%@ page language="java" import="java.util.*" pageEncoding="UTF-8" contentType="text/html; charset=ISO-8859-1"%>
...
<%
		try {
			...
			String inningId = session.getAttribute("InningId").toString();
			...
			vSpParam = new Vector();
			vSpParam.add(inningId);
			vSpParam.add(player1);
			vSpParam.add(player2);
			vSpParam.add(reportType);
			crsPointsDetail = generateStProc.GenerateStoreProcedure("esp_dsp_ball_pitch_report", vSpParam, "ScoreDB");
			
			vSpParam = new Vector();
			vSpParam.add(player1);
			profileDetail = generateStProc.GenerateStoreProcedure("data.esp_dsp_playerProfDetails", vSpParam, "ScoreDB");


			boolean flag = false;
			char rightHanded = 'Y';
			String side1_label = "Right handed bowler";
			
			if(profileDetail.next()) {
				if(profileDetail.getObject("bowl") != null) {
					rightHanded = profileDetail.getObject("bowl").toString().charAt(0);	
				}
			}
			
			if(reportType.equals("1") && rightHanded == 'N') {
				side1_label = "Left handed bowler";
			}
			
			if(reportType.equals("1")) {
				g.setColor(Color.BLACK);
				g.drawString(side1_label, 17, 230);		
			}
