...
<%
	try{
		 ...
		 CachedRowSet matchResultCatachedRowSet = null;
         String result = null;
       ...
       vparam.add(matchId);
        matchResultCatachedRowSet = lobjGenerateProc.GenerateStoreProcedure("esp_dsp_matchstatus",vparam,"ScoreDB");	
        vparam.removeAllElements();
            
        if(matchResultCatachedRowSet!=null && matchResultCatachedRowSet.size() > 0){
			 try{
			
				while(matchResultCatachedRowSet.next()){ 
				result =  matchResultCatachedRowSet.getString("result")==null?"0":matchResultCatachedRowSet.getString("result");
				}
			 }catch(Exception e){
				e.printStackTrace();
...
System.out.print("resultresultresult"+result);
%>
<% }	
%>
...
function Inning(id){
	...
	if (xmlHttp==null){
      ...
      mdiv.innerHTML="<center><img src='../images/ajaxbuzyicon.gif' width=50em;height=50em ></img></center>";
...
function callRefresh(){
	document.main.action="/cims/jsp/InningsDetails.jsp"
			...
			<td>
				<table border="0">
					<tr style="width: 100%" align="center">
						<td>
							<input class="btn btn-warning btn-small" type="button" value="Refresh" onclick="callRefresh()"/>
							 <input class="btn btn-warning btn-small" type="button" name="btnprint" id="btnprint" value="Print" onclick="printreport()">
						</td>
					</tr>
					<tr>
						<td>
